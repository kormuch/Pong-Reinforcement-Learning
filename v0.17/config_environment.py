...
class EnvConfig:
    ...
    COMPUTER_DIFFICULTY = 'easy'  # Options: 'easy', 'medium', 'hard'
    COMPUTER_SPEEDS = {
        'easy': 3,      # Slower, more beatable
        'medium': 4,    # Balanced, challenging but fair
        'hard': 5       # Fast but still beatable
    }
    COMPUTER_REACTION_ENABLED = True  # CPU only moves when ball crosses midline
    
    # RL agent settings (for reinforcement learning)
    AI_DIFFICULTY = 'medium'  # For future RL agent training
    AI_SPEEDS = {
        'easy': 3,      # For RL training
        'medium': 4,    # For RL training
        'hard': 5       # For RL training
    }
    ...
    # Edge hit mechanics
    EDGE_HIT_ANGLE_BOOST = True  # Edge hits create steeper angles
    EDGE_HIT_SPEED_BOOST = 1.0  # No extra speed boost (rally system handles speed)
    EDGE_HIT_STEEP_ANGLE = EDGE_HIT_ANGLE_BOOST  # For compatibility with simulator
    ...
    STATE_VALUE_AI = 0.5        # Value for CPU paddle pixels (distinguishable!)
    ...
    def get_env_params(cls):
        ...
        return {
            ...
            'cpu_difficulty': cls.COMPUTER_DIFFICULTY
        }
