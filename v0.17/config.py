...
def validate_config():
    ...
    if EnvConfig.COMPUTER_DIFFICULTY not in EnvConfig.COMPUTER_SPEEDS:
        raise ValueError(f"CPU difficulty must be one of {list(EnvConfig.COMPUTER_SPEEDS.keys())}")
...
def print_config_summary():
    ...
    print(f"  Difficulty: {EnvConfig.COMPUTER_DIFFICULTY}")
    print(f"  Speed: {EnvConfig.COMPUTER_SPEEDS[EnvConfig.COMPUTER_DIFFICULTY]} pixels/step")
    print(f"  Reaction Rule: {'Midline-based' if EnvConfig.COMPUTER_REACTION_ENABLED else 'Always active'}")
...
def get_all_params():
    ...
    return {
        'environment': {
            ...
            'cpu_difficulty': EnvConfig.COMPUTER_DIFFICULTY,
